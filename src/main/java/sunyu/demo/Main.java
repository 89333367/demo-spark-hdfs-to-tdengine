...
public class Main {
    ...
    public static void main(String[] args) {
        // args
        //# startDay endDay partitions killData
        String hdfsPath;
        String startDay;
        String endDay;
        int partitions;
        ...
        if (args == null) {
            ...
            startDay = args[0];
            endDay = args[1];
            partitions = Convert.toInt(args[2], 100);
            killData = Convert.toBool(args[3], false);
        }
        ...
        for (DateTime dateTime : DateUtil.range(DateUtil.parse(startDay), DateUtil.parse(endDay), DateField.DAY_OF_YEAR)) {
            l.add(dateTime);
        }
        for (DateTime dateTime : CollUtil.reverse(l)) {//倒序处理
